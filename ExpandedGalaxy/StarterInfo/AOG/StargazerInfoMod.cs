...
namespace ExpandedGalaxy
{
    ...
    internal class StargazerInfoMod
    {
        ...
        internal static void Postfix(PLStarGazerInfo __instance, bool previewStats, bool startingPlayerShip)
        {
            if (!bypass && PhotonNetwork.offlineMode && previewStats)
            {
                bypass = true;
                PLUICreateGameMenu.Instance.StartCoroutine(StarterInfo.DelayedSetupShipStatsPostfix(2, __instance, previewStats, startingPlayerShip));
                return;
            }
            bypass = false;
