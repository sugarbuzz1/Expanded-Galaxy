...
namespace ExpandedGalaxy
{
    ...
    internal class CruiserInfoMod
    {
        ...
        internal static void Postfix(PLCruiserInfo __instance, bool previewStats, bool startingPlayerShip)
        {
            if (!bypass && PhotonNetwork.offlineMode && previewStats)
            {
                bypass = true;
                PLUICreateGameMenu.Instance.StartCoroutine(StarterInfo.DelayedSetupShipStatsPostfix(0, __instance, previewStats, startingPlayerShip));
                return;
            }
            bypass = false;
            ...
            try
            {
                ...
                __instance.MyStats.RemoveShipComponent(__instance.MyStats.GetShipComponent<PLMegaTurret>(ESlotType.E_COMP_MAINTURRET));
                ...
                else
                {
                    ...
                    if ((double)(float)PLServer.Instance.ChaosLevel > 1.0)
                    {
                        ...
                        if (num >= 0.85)
                        {
                            ...
                            bool flag = (double)(float)PLServer.Instance.ChaosLevel > 2.0 && (int)deterministicRand.Next(3) % 3 == 0;
                            if (flag)
                        ...
                        float num2 = Mathf.Clamp01((float)((double)deterministicRand.NextFloat() + (double)(float)PLServer.Instance.ChaosLevel * 0.04));
