    ...
    internal class RecieveSyncCheck : ModMessage
    {
        public override void HandleRPC(object[] arguments, PhotonMessageInfo sender)
        {
            ...
            if (!sender.sender.IsMasterClient)
            {
                ...
                if (flag)
                {
                    Messaging.Notification("Player [" + Systems.GetPlayerFromPhotonPlayer(sender.sender).GetPlayerName() + "] joined with mismatch component info!\nSee ship log for details.", PLNetworkManager.Instance.LocalPlayer.GetPhotonPlayer(), durationMs: 30000);
                    Messaging.ShipLog("Player [" + Systems.GetPlayerFromPhotonPlayer(sender.sender).GetPlayerName() + "] joined with mismatch component info!", tag: "ExGal.", color: Color.grey);
                ...
                {
                    Messaging.Notification("Player [" + Systems.GetPlayerFromPhotonPlayer(sender.sender).GetPlayerName() + "] synced correctly!", PLNetworkManager.Instance.LocalPlayer.GetPhotonPlayer(), durationMs: 6000);
                }
            }
        }
    }
